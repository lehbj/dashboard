...
class Datenbank:
    ...
    def _tabellen_erstellen(self) -> None:
        ...
        with self._connection:
            self._connection.executescript(
                'CREATE TABLE IF NOT EXISTS studium(id INTEGER PRIMARY KEY AUTOINCREMENT, studiengang TEXT NOT NULL, hochschule TEXT NOT NULL, start_datum date NOT NULL, geplantes_end_datum date NOT NULL);')
            self._connection.executescript(
                'CREATE TABLE IF NOT EXISTS semester (id INTEGER PRIMARY KEY AUTOINCREMENT, studium_id INTEGER NOT NULL, nummer INTEGER NOT NULL, FOREIGN KEY (studium_id) REFERENCES studium(id) ON DELETE CASCADE);')
            self._connection.executescript(
                'CREATE TABLE IF NOT EXISTS modul (id INTEGER PRIMARY KEY AUTOINCREMENT, semester_id INTEGER NOT NULL, kuerzel TEXT NOT NULL, name TEXT NOT NULL, etcs INTEGER NOT NULL, FOREIGN KEY (semester_id) REFERENCES semester(id) ON DELETE CASCADE);')
            self._connection.executescript(
                'CREATE TABLE IF NOT EXISTS pruefung (id INTEGER PRIMARY KEY AUTOINCREMENT, modul_id INTEGER NOT NULL UNIQUE, note FLOAT(1,2) NOT NULL, FOREIGN KEY (modul_id) REFERENCES modul(id) ON DELETE CASCADE);')
    ...
    def studium_aendern(self, studium: Studium) -> None:
        ...
        """
        self.write(
            f'UPDATE studium SET studiengang="{studium.studiengang}", hochschule="{studium.hochschule}", start_datum="{studium.start_datum}", geplantes_end_datum="{studium.geplantes_end_datum}";')
